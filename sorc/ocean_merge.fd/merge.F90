...
500   format("UPDATE_MASK: Total lake point", 1x, i6, 1x, "where", 1x, i6, 1x, "has no depth.")
   ...
   subroutine read_lndinfo(tile, merge_attrs, nml_attrs)
      ...
      integer(ilong) :: ncid, dimid, varid
      ...
      call ncerrhdl(nf90_open(ncfile, nf90_nowrite, ncid))
      call ncerrhdl(nf90_inq_dimid(ncid, trim(adjustl(nml_attrs%nml_lake%nlon_str)), dimid))
      call ncerrhdl(nf90_inquire_dimension(ncid, dimid, len=merge_attrs%nlon))
      call ncerrhdl(nf90_inq_dimid(ncid, trim(adjustl(nml_attrs%nml_lake%nlat_str)), dimid))
      call ncerrhdl(nf90_inquire_dimension(ncid, dimid, len=merge_attrs%nlat))
      call init_struct(merge_attrs)
      ncvar = "geolon"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%lon2d))
      ncvar = "geolat"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%lat2d))
      ncvar = "lake_frac"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%lake_frac))
      ncvar = "lake_depth"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%lake_depth))
      ncvar = "land_frac"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%land_frac))
      ncvar = "slmsk"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%slmsk))
...
502   format("READ_LNDINFO: Reading netCDF variable", 1x, a, ".")
   ...
   subroutine read_ocninfo(tile, merge_attrs, nml_attrs)
      ...
      write (ncfile, 500) trim(adjustl(nml_attrs%nml_ocean%ocean_mask_dir)), &
         trim(adjustl(nml_attrs%nml_lake%res)), trim(adjustl(nml_attrs%nml_ocean%res)), &
         tile
      write (6, 501) trim(adjustl(ncfile))
      call ncerrhdl(nf90_open(ncfile, nf90_nowrite, ncid))
      call ncerrhdl(nf90_inq_dimid(ncid, trim(adjustl(nml_attrs%nml_ocean%nlon_str)), dimid))
      call ncerrhdl(nf90_inquire_dimension(ncid, dimid, len=merge_attrs%nlon))
      call ncerrhdl(nf90_inq_dimid(ncid, trim(adjustl(nml_attrs%nml_ocean%nlat_str)), dimid))
      call ncerrhdl(nf90_inquire_dimension(ncid, dimid, len=merge_attrs%nlat))
      call init_struct(merge_attrs)
      ncvar = "land_frac"
      write (6, 502) trim(adjustl(ncvar))
      call ncerrhdl(nf90_inq_varid(ncid, trim(adjustl(ncvar)), varid))
      call ncerrhdl(nf90_get_var(ncid, varid, merge_attrs%ocn_frac))
      call ncerrhdl(nf90_close(ncid))
...
502   format("READ_OCNINFO: Reading netCDF variable", 1x, a, ".")
   ...
   subroutine write_maskupdate(tile, merge_attrs, nml_attrs)
      ...
      write (ncfile, 500) trim(adjustl(nml_attrs%nml_info%out_dir)), &
         trim(adjustl(nml_attrs%nml_lake%res)), trim(adjustl(nml_attrs%nml_ocean%res)), &
         tile
